...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/weed_labels.txt"
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        int8_t *box_tensor = (int8_t *)((rknn_output *)outputs)[box_idx].buf;
        int8_t *score_tensor = (int8_t *)((rknn_output *)outputs)[score_idx].buf;
        int8_t *score_sum_tensor = (int8_t *)((rknn_output *)outputs)[score_sum_idx].buf;
        ...
        int32_t score_sum_zp = app_ctx->output_attrs[score_sum_idx].zp;
        float score_sum_scale = app_ctx->output_attrs[score_sum_idx].scale;

        int grid_h = app_ctx->output_attrs[box_idx].dims[2];
        int grid_w = app_ctx->output_attrs[box_idx].dims[3];
        int stride = model_in_w / grid_w;

        validCount += process_i8(
            box_tensor, box_zp, box_scale,
